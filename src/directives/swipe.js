...
function Swipe (container, options = {}) {
  ...
  if (element.children.length === 2) {
    // 目前针对子项为2个的时候，强制开启往返轮播机制
    options.carousel = false
  }
  if (options.carousel) {
    // 新增carousel入参启用新轮播机制，关闭原本默认的循环轮播方式
    // by KyLeo 2017.08.16
  ...
  function setup () {
    ...
    if (options.continuous && browser.transitions && slides.length < 3) {
  ...
  function prev () {
    (options.continuous || index) && slide(index - 1)
  }

  let direction = 1 // 往返方向，初始值为1，1代表向右，-1代表向左 ---- by KyLeo 2017.08.16
  function next () {
    let length = slides.length
    if (options.continuous || index < length - 1) {
      slide(index + 1)
    } else if (options.carousel && length === 2) {
      // 新增的往返轮播方式 ---- by KyLeo 2017.08.16
      if (direction === 1) {
        if (index + 1 === length) {
          // 到达最右边
          direction = -1
  ...
  let events = {
    ...
    transitionEnd: function (event) {
      if (parseInt(event.target.getAttribute('data-index'), 10) === index) {
        delay && begin()
        options.transitionEnd && options.transitionEnd.call(event, index)
      }
    }
...
let swipe = {
  inserted (el, binding) {
    el._options = binding.value.options || {}
    el._swipeObj = Swipe(el, el._options)
  },
  unbind (el) {
    el._swipeObj.kill()
  }
